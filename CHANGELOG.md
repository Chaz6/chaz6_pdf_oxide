...
- **@erichevers** — #1031 (image XObjects with indirect /Width and /Height references are dropped), #1030 (CCITTFaxDecode images with no explicit /K render blank), #1033 (marked-content scope leaks across prescan region boundaries)
...
- **@Goldziher** — #975 (strict table extraction merges adjacent columns in dense-pitch tables), #1029 (180-degree rotated runs report rotation_degrees=0 and get merged), #1034 (extract_cell hardcodes rotation_degrees to 0 for structure-tree-derived cells)
